...
class URLShortenerTestCase(TestCase):
    ...
    def test_get_url_shortener(self):
        ...
        us_settings.URL_SHORTENER_BACKEND = 'gironimo.blog.tests.custom_url_shortener'
        try:
            with warnings.catch_warnings(record=True) as w:
                self.assertEquals(get_url_shortener(), default_backend)
                self.assertTrue(issubclass(w[-1].category, RuntimeWarning))
        ...
        us_settings.URL_SHORTENER_BACKEND = 'gironimo.blog.url_shortener'\
                                            '.backends.default'
        self.assertEquals(get_url_shortener(), default_backend)
