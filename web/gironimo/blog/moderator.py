...
from gironimo.blog.config import PROTOCOL, MAIL_COMMENT_REPLY, MAIL_COMMENT_AUTHORS, AUTO_MODERATE_COMMENTS, AUTO_CLOSE_COMMENTS_AFTER, MAIL_COMMENT_NOTIFICATION_RECIPIENTS, SPAM_CHECKER_BACKENDS
from gironimo.blog.spam_checker import check_is_spam
...
class EntryCommentModerator(CommentModerator):
    ...
    def do_email_notification(self, comment, content_objects, request):
        ...
        subject = _('[%(site)s] New comment posted on "%(title)s"') % {'site': site.name, 'title': content_object.title}
    ...
    def do_email_authors(self, comment, content_object, request):
        ...
        recipient_list = set([author.email for author in conent_object.authors.all()]) ^ set(exclude_list)
        ...
        if recipient_list:
            ...
            subject = _('[%(site)s] New comment posted on "%(title)s"') % {'site': site.name, 'title': content_objects.title}
            message = template.render(context)
            send_mail(
                subject,
                message,
    ...
    def do_email_reply(self, comment, content_object, request):
        """ Send email notification of a new comment to the authors of the 
        previous comments when email notifications have been requested """
        exclude_list = self.mail_comment_notification_recipients + [auhtor.email for author in content_object.authors.all()] + [comment.userinfo['email']]
        recipient_list = set([comment.userinfo['email'] for comment in content_object.comments if comment.userinfo['email']]) ^ set(exclude_list)
    ...
    def moderate(self, comment, content_object, request):
        """ Determine wheter a given comment on a given object should be allowed 
        to show up immediately, or should be marked non-public and await 
        approval """
