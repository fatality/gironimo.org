...
from gironimo.blog.comparison import VectorBuilder, pearson_score
...
VECTORS = None
VECTORS_FACTORY = lambda: VectorBuilder(Entry.published.all(), ['title', 'excerpt', 'content'])
...
# TODO: integrate django_hits here !!!
@register.inclusion_tag('blog/tags/dummy.html')
...
def get_similar_entries(context, number=5, template='blog/tags/similar_entries.html', flush=False):
...
def get_archives_entries(template='blog/tags/archives_entries.html'):
    ...
    return {
        'template': template,
        'archives': Entry.published.dates('created', 'month', order='DESC')
    }


@register.inclusion_tag('blog/tags/dummy.html')
def get_archives_entries_tree(template='blog/tags/archives_entries_tree.html'):
    """ Return archives entries as a Tree """
    return {
        'template': template,
        'archives': Entry.published.dates('created', 'day', order='ASC')
    }


@register.inclusion_tag('blog/tags/dummy.html', takes_context=True)
def get_calendar_entries(context, year=None, month=None, template='blog/tags/calendar.html'):
    """ Return an HTML calendar of entries """
    if not year or not month:
        date_month = context.get('month') or context.get('day') or getattr(context.get('object'), 'creation_date', None) or datetime.today()
        year, month = date_month.timetuple()[:2]
    
    calendar = GironimoCalendar()
    current_month = datetime(year, month, 1)
    
    dates = list(Entry.published.dates('created', 'month'))
...
@register.inclusion_tag('blog/tags/dummy.html')
def get_recent_linkbacks(number=5, template='blog/tags/recent_linkbacks.html'):
...
@register.inclusion_tag('blog/tags/dummy.html', takes_context=True)
def blog_pagination(context, page, begin_pages=3, end_pages=3, before_pages=2, after_pages=2, template='blog/tags/pagination.html'):
    ...
    middle = page.paginator.page_range[max(page.number - before_pages - 1, 0): page.number + after_pages]
...
@register.inclusion_tag('blog/tags/dummy.html', takes_context=True)
def blog_breadcrumbs(context, separator='/', root_name='Blog', template='blog/tags/breadcrumbs.html',):
...
def get_gravatar(email, size=80, rating='g', default=None):
    ...
    url = 'http://www.gravatar.com/avatar/%s.jpg' % md5(email.strip().lower()).hexdigest()
    options = {
        's': size, 
        'r': rating
    }
...
def get_tags(parser, token):
    ...
    if len(bits) != 3:
        raise TemplateSyntaxError('get_tags tag takes exactly two arguments')
    if bits[1] != 'as':
        raise TemplateSyntaxError("first argument to get_tags tag must be 'as'")
