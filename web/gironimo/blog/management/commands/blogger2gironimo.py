...
class Command(NoArgsCommand):
    """ Command object for importing a Blogger blog via Google's gdata API."""
    help = 'Import a Blogger blog.'
    ...
    def handle_noargs(self, **options):
        ...
        self.write_out(self.style.TITLE(
            'Starting migration from Blogger'))
        ...
        if default_author:
            try:
                ...
                raise CommandError(
                    'Invalid username for default author "%s"' % \
                    default_author)
        else:
