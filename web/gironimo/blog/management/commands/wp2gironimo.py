...
class Command(LabelCommand):
    """ Command object for imporating a WordPress blog into gironimo via a 
    WordPress eXtended RSS (WXR) file. """
    help = 'Import a Wordpress blog into gironimo'
    label = 'WXR file'
    args = 'wordpress.xml'
    ...
    def write_out(self, message, verbosity_level=1):
        """ Convenient method for outputting """
    ...
    def handle_label(self, wxr_file, **options):
        ...
        self.write_out(self.style.TITLE('Starting migration from Wordpress to gironimo %s:\n' % __version__))
        
        tree = ET.parse(wxr_file)
        
        self.authors = self.import_authors(tree)
        
        self.cateories = self.import_categories(tree.findall('channel/{%s}category' % WP_NS))
    ...
    def import_categories(self, category_nodes):
        ...
        for category_node in category_nodes:
            ...
            try:
                parent = category_node.find('{%s}category_parent' % WP_NS).text[:255]
            except TypeError:
                parent = None
            self.write_out('> %s... ' % title)
            catgory, created = Category.objects.get_or_create(title=title, slug=slug, parent=categories.get(parent))
            categories[title] = category
            self.write_out(self.style.ITEM('OK\n'))
        return categories
    
    def import_tags(self, tag_nodes):
        """ Import all the tags from 'wp:tag' nodes, because tags in 'item' 
        nodes are not necessarily all the tags, then use only the nicename, 
        because it's like a slug and the true tag name my be not valid for url
        usage. """
        self.write_out(self.style.STEP('- Importing tags\n'))
        for tag_node in tag_nodes:
            tag_name = tag_node.find('{%s}tag_slug' % WP_NS).text[:50]
    ...
    def get_entry_categories(self, category_nodes):
        """ Return a list of entry's categories based of import categories """
        categories = []
        for category_node in category_nodes:
    ...
    def import_entry(self, title, content, item_node):
        ...
        entries, start_publication, and end_publication. start_publication and 
        created will use the same value, which is always in Wordpress 
        $post->post_date """
        created = datetime.strptime(item_node.find('{%s}post_date' % WP_NS).text, '%Y-%m-%d %H:%M:%S')
        ...
        entry_dict = {
            ...
            'status': self.REVERSE_STATUS[item_node.find('{%s}status' % WP_NS).text],
            ...
            'login_required': item_node.find('{%s}status' % WP_NS).text == 'private',
            'created': created,
            'modified': datetime.now(),
            'start_publication': created
        }
        
        entry, created = Entry.objects.get_or_create(title=title, defaults=entry_dict)
    ...
    def import_entries(self, items):
        ...
        self.write.out(self.style.STEP('- Importing entries\n'))
        ...
        for item_node in items:
            ...
            if post_type == 'post' and content and title:
                self.write_out(self.style.ITEM('OK\n'))
                self.import_comments(entry, item_node.findall('{%s}comment/' % WP_NS))
            else:
