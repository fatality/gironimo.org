...
class Command(LabelCommand):
    """ Command object for importing a WordPress blog via a WordPress eXtended 
    RSS (WXR) file. """
    help = 'Import a Wordpress blog.'
    label = 'WXR file'
    args = 'wordpress.xml'
    ...
    def handle_label(self, wxr_file, **options):
        ...
        self.write_out(self.style.TITLE(
            'Starting migration from Wordpress'))
    ...
    def import_tags(self, tag_nodes):
        """ Import all the tags form 'wp:tag' nodes, because tags in 'item' 
        nodes are not necessarily all the tags, then use only the nicename, 
        because it's like a slug and the true tag name may be not valid for url 
        usage."""
    ...
    def import_entry(self, title, content, item_node):
        ...
        creation_date will use the same value, wich is always in Wordpress 
        $post->post_date """
    ...
    def import_comments(self, entry, comment_nodes):
        """ Loops over comments nodes and import then in 
