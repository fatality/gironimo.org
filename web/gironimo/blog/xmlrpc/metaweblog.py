...
def blog_structure(site):
    """ A blog structure """
    return {
        'url': '%s://%s%s' % (PROTOCOL, site.domain, reverse('blog_entry_archive_index')),
...
def user_structure(user, site):
    ...
    return {
        ...
        'url': '%s://%s%s' % (PROTOCOL, site.domain, reverse('blog_author_detail', args=[user.username]))
    }
...
def new_post(blog_id, username, password, post, publish):
    ...
    if 'categories' in post:
        entry.categories.add(*[Category.objects.get_or_create(title=cat, slug=slugify(cat))[0] for cat in post['categories']])
