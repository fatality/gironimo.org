...
class EntryAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ...
        (_('Publication'), {
                'fields': ('categories', 'tags', 'sites', 'slug',)
        })
    )
    list_filter = (
        'categories', 'authors', 'status', 'featured', 'login_required', 
        'comment_enabled', 'pingback_enabled', 'created', 'start_publication', 
        'end_publication', 'sites',
    )
    list_display = (
        'get_title', 'get_authors', 'get_categories', 'get_tags', 'get_sites', 
        'get_comments_are_open', 'pingback_enabled', 'get_is_actual', 
        'get_is_visible', 'get_link', 'get_short_url', 'created',
    )
    radio_fields = {'template': admin.VERTICAL}
    filter_horizontal = ('categories', 'authors', 'related',)
    prepopulated_fields = {'slug': ('title',)}
    search_fields = ('title', 'exceprt', 'content', 'tags',)
    actions = [
        'make_mine', 'make_published', 'make_hidden', 'close_comments', 
        'close_pingbacks', 'ping_directories', 'make_tweet', 'put_on_top',
    ]
    actions_on_top = True
    ...
    def get_title(self, entry):
        """ Retrun the title with word count and number of comments """
        title = _('%(title)s (%(word_count)i words)') % {'title': entry.title, 'word_count': entry.word_count}
        comments = entry.comments.count()
        if comments:
            return _('%(title)s (%(comments)i comments)') % {'title': title, 'comments': comments}
        return title
    get_title.short_description = _('title')
    
    def get_authors(self, entry):
        """ Return the authors ini HTML """
        try:
            authors = ['<a href="%s" target="blank">%s</a>' % (reverse('blog_author_detail', args=[author.username]), author.username) for author in entry.authors.all()]
    ...
    def get_categories(self, entry):
        ...
        try:
            categories = ['<a href="%s" target="blank">%s</a>' % (category.get_absolute_url(), category.title) for category in entry.categories.all()]
        except NoReverseMatch:
    ...
    def get_tags(self, entry):
        ...
        try:
            return ', '.join(['<a href="%s" target="blank">%s</a>' % (reverse('blog_tag_detail', args=[tag.name]), tag.name) for tag in Tag.objects.get_for_object(entry)])
        except NoReverseMatch:
            return entry.tags
    ...
    def get_sites(self, entry):
        ...
        return ', '.join(['<a href="http://%(domain)s" target="blank">%(name)s</a>' % site.__dict__ for site in entry.sites.all()])
    ...
    def get_link(self, entry):
        ...
        return u'<a href="%s" target="blank">%s</a>' % (entry.get_absolute_url(), _('View'))
    ...
    def get_short_url(self, entry):
        ...
        return '<a href="%(url)s" target="blank">%(url)s</a>' % {'url': short_url}
    get_short_url.allow_tags = True
    get_short_url.short_description = _('short url')
    
    def save_model(self, request, entry, form, change):
        """ Save the authors, update time, make an exceprt """
    ...
    def get_actions(self, request):
        ...
        if not request.user.has_perm('blog.can_change_author') or not request.user.has_perm('blog.can_view_all'):
    ...
    def make_tweet(self, request, queryset):
        ...
        auth.set_acces_token(config.TWITTER_ACCES_KEY, config.TWITTER_ACCES_SECRET)
        api = tweepy.API(auth)
        for entry in queryset:
            short_url = entry.short_url
            message = '%s %s' % (entry.title[:139 - len(short_url)], short_url)
            api.update_status(message)
        self.message_user(request, _('The selected entries have beeen tweeted.'))
    ...
    def put_on_top(self, request, queryset):
        ...
        queryset.update(created=datetime.now())
        self.ping_directories(request, queryset, messages=False)
        self.message_user(request, _('The selected entries are now at the current date.'))
    put_on_top.short_description = _('Put the selected entries on top at the current date')
    
    def ping_directories(self, request, queryset, messages=False):
        ...
        for directory in config.PING_DIRECTORIES:
            ...
            if messages:
                ...
                if success:
                    self.message_user(request, _('%(directory)s directory succesfully pinged %(success)d entries.') % {'directory': directory, 'success': success})
    ...
    def _media(self):
        ...
        media = super(EntryAdmin, self).media + Media(
            css={
                'all': ('%scss/jquery.autocomplete.css' % STATIC_URL,)
            },
            js=(
                '%sjs/jquery.js' % STATIC_URL,
        ...
        if config.WYSIWYG == 'wymeditor':
            media += Media(
                js=(
                    '%sjs/wymeditor/jquery.wymeditor.pack.js' % STATIC_URL,
                    '%sjs/wymeditor/jquery.plugins/hovertools/jquery.wymeditor.hovertools.js' % STATIC_URL,
            ...
            media += Media(
                js=(
                    ...
                    '%sjs/markitup/sets/%s/set.js' % (STATIC_URL, config.MARKUP_LANGUAGE),
                    reverse('admin:blog_entry_markitup')
                ),
                css={
                    'all': (
                        '%sjs/markitup/skins/django/style.css' % STATIC_URL,
                        '%sjs/markitup/sets/%s/style.css' % (STATIC_URL, config.MAKRUP_LANGUAGE)
                    )
                }
            )
        return media
