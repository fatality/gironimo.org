...
class TreeNodeChoiceField(forms.ModelChoiceField):
    ...
    def label_from_instance(self, obj):
        ...
        return u'%s %s' % (self.level_indicator * getattr(obj, obj._mptt_meta.level_attr), smart_unicode(obj))
...
class MPTTFilteredSelectMultiple(widgets.FilteredSelectMultiple):
    ...
    def render_options(self, choices, select_choices):
        ...
        this field, make sure they have the extra tuple too! """
        def render_option(option_value, option_label, sort_fields):
            """ Inner scope render option """
            option_value = force_unicode(option_value)
            selected_html = (option_value in selected_choices) and u' selected="selected"' or ''
            return u'<option value="%s" data-tree-id="%s" data-left-value="%s"%s>%s</option>' % (escape(option_value), sort_fields[0], sort_fields[1], selected_html, conditional_escape(force_unicode(option_label)),)
    ...
    class Media:
        """ MPTTFilteredSelectedMultiple's Media """
        js = (settings.ADMIN_MEDIA_PREFIX + 'js/core.js',
              settings.STATIC_URL + 'lib/js/mptt_m2m_selectbox.js',
