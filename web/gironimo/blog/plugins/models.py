...
from gironimo.blog.plugins.settings import PLUGINS_TEMPLATES


TEMPLATES = [
    ('blog/cms/entry_list.html', _('Entry list (default)')),
...
class LatestEntriesPlugin(CMSPlugin):
    ...
    def render_template(self):
        """ Override render_template to use the template_to_render attribute """
...
def invalidate_menu_cache(sender, **kwargs):
    """Signal receiver to invalidate the menu_pool cache when an entry is posted"""
    menu_pool.clear()


post_save.connect(invalidate_menu_cache, sender=Entry, dispatch_uid='gironimo.blog.entry.postsave.invalidate_menu_cache')
post_delete.connect(invalidate_menu_cache, sender=Entry, dispatch_uid='gironimo.blog.entry.postdelete.invalidate_menu_cache')
