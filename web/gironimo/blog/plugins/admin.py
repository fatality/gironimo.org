...
class EntryPlaceholderAdmin(PlaceholderAdmin, EntryAdmin):
    ...
    fieldsets = (
        ...
        (_('Content'), {
            'fields': ('content_placeholder',),
            'classes': ('plugin-holder', 'plugin-holder-nopage')
        }),
        (_('Options'), {
            'fields': ('featured', 'excerpt', 'template', 'related', 'authors',
                       'created', 'start_publication', 'end_publication',),
        ...
        (_('Publication'), {
            'fields': ('sites', 'categories', 'tags', 'slug',)
        })
    )
...
if ENTRY_BASE_MODEL == 'gironimo.blog.plugins.placeholder.EntryPlaceholder':
